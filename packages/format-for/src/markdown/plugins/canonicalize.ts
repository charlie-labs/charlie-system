...
function mentionData(
  ...
  id?: string,
  label?: string
): NonNullable<MentionNode['data']> {
  const data: NonNullable<MentionNode['data']> = { subtype };
    ...
    visit(
      ...
      (node: Html, index: number | undefined, parent: Parent | undefined) => {
        ...
        if (m) {
          const mention: MentionNode = {
            type: 'mention',
            data: mentionData('channel', m[1], m[2]),
            children: [],
          };
          parent.children.splice(index, 1, mention);
          return [CONTINUE, index + 1];
      ...
      (node: Text, index: number | undefined, parent: Parent | undefined) => {
        ...
        while ((m = re.exec(input))) {
          ...
          if (m[1]) {
            ...
            const mention: MentionNode = {
              type: 'mention',
              data: mentionData('channel', m[4], m[5]),
              children: [],
            };
            fragments.push(mention);
          } else if (m[6]) {
