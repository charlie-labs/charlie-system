  ...
  async function refreshLinear(): Promise<void> {
    ...
    inflightLinear = (async () => {
      ...
      try {
        ...
        if (lin.orgSlug !== undefined) snap.orgSlug = lin.orgSlug;
        linearSnap = snap;
        try {
...
async function fetchAllSlackUsers(
  ...
  for (let page = 0; page < SLACK_MAX_PAGES; page++) {
    ...
    for (const mm of members) {
      ...
      const user = label === undefined ? { id } : { id, label };
      ...
      if (label) {
        ...
        if (key && !users[key]) users[key] = user;
      }
    }
...
async function fetchLinearDefaults(apiKey: string): Promise<{
  ...
  for (const uu of usersNodes) {
    ...
    if (!url) continue;
    const user = label === undefined ? { url } : { url, label };
