 ...
 *
 * Typical usage:
 *   import type { CfgFlags } from '@charlie-labs/oclif-plugin-helpers-zod3';
...
/** Any Zod 3 schema, while preserving concrete input/output types in generics. */
...
function makeManifest<D extends Defs, Obj extends AnyZodType>(
  ...
  const manifest: FlagManifest<D, Obj> = {
    ...
    schema,
    parse(raw) {
      // Zod 3 distinguishes missing object properties from properties whose
